...
int main()
{
	...
	camera = new Camera(glm::vec3(0, 0, -3), 0, 0, 45.0f, (float)screenWidth / screenHeight);

	glBindVertexArray(VAO);
	unsigned int transformLoc = glGetUniformLocation(ourShader.ID, "transformMVP");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (unsigned int i = 0; i < 10; i++)
		{
			...
			modelTrans = glm::rotate(modelTrans, (float)glfwGetTime() / 100, glm::vec3(1.0f, 0.0f, 0.0f));
			modelTrans = glm::rotate(modelTrans, (float)glfwGetTime() / 150, glm::vec3(0.0f, 1.0f, 0.0f));
